...
public class XMLSerializer {
	 ...
	 */
	public XMLSerializer (TransitivityResult result, Writer writer) {
		this.result = result;
		this.writer = writer;
	}
	...
	protected StringBuffer writeCompositeEntity (CompositeEntity ce) {
		...
		if (isFullyMatchConnected(ce)) sb.append ("<FullyMatchConnected/>");
		else if (isMatchConnected(ce)) sb.append ("<MatchConnected/>");
		else sb.append ("<HoldConnected/>");
		...
		for (int i=0; i<children.size(); i++) {
			...
			if (child instanceof CompositeEntity) composites.add(child);
		}
		if (composites.size() > 0) sb.append(writeSubGraphs(composites));
	...
	private StringBuffer writeConnections (CompositeEntity ce){
		...
		for (int i=0; i<links.size(); i++) {
			...
			if (node1 instanceof CompositeEntity || node2 instanceof CompositeEntity) {
				sb.append(writeAggregateConnection (link));
			} else {
				List mrs = link.getLinkDefinition();
				if (mrs.size() > 1) throw new IllegalArgumentException 
					("Simple link size > 1.  Link between" + link.getNode1().getNodeId() + " " +
	...
	private String writeDecision (MatchRecord2 mr) {
		if (mr.getMatchType() == MatchRecord2.MATCH) return "match";
		else if (mr.getMatchType() == MatchRecord2.HOLD) return "hold";
		else return "error";
	}
	...
	private StringBuffer writeAggregateConnection (Link link) {
		...
		else sb.append(writeRecord(node1.getNodeId()));

		if (node2 instanceof CompositeEntity) 
			sb.append(writeCompositeNode(node2.getNodeId()));
		else sb.append(writeRecord(node2.getNodeId()));
	...
	private boolean isFullyMatchConnected (CompositeEntity ce) {
		...
		if (out.getChildren().size() != ce.getChildren().size()) return false;
