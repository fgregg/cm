...
public class MatchSchedulerIT {
	...
	public static final String LOG_SOURCE = MatchSchedulerIT.class.getSimpleName();
	...
	public void testStartOABA(final String tag, final String externalId,
		...
		logger.info("Checking matchDedupQueue");
		JMSConsumer consumer = jmsContext.createConsumer(matchDedupQueue);
		OabaJobMessage startData = receiveStartData(consumer, VERY_LONG_TIMEOUT_MILLIS);
		logger.info(JmsUtils.queueInfo("Received from: ", matchDedupQueue, startData));
		if (startData == null) {
		...
		assertTrue(processingEntry.getCurrentProcessingEventId() == EVT_DONE_MATCHING_DATA);
		...
		assertTrue(updateMessage.getPercentComplete() == PCT_DONE_MATCHING_DATA);
