 ...
 * LinkCriteria lk = new LinkCriteria(GraphProperty.BCM,true);//true - must include query record
 *   	
 ...
 * </pre>
 * <p>
 * Each EvaluatedRecord in the returned array consists of a matched record and a score reflecting the likelihood of a match.
 * In contrast to the result of <code>getMatchCandidates</code> each returned evaluated record could be either linked record set or a single recors.
 * If a link criteria is passed to the method requires linked record sets to include query record then the output (by the definition of the linked record set)
 * will contain only one linked set. Otherwise it is possible that the result will include more then one linked record set.
 *      
 * 
 * @author emoussikaev
...
public interface OnlineMatchAnalyzer extends EJBObject {
	 ...
	 * The exact meaning of being <i>tightly connected together</i> is defined by the link criteria <code>c</code>.
	 * Records placed in one group are recommended for merging. The result array of evaluated record
	 * includes extracted <code>LinkedRecordSets</code> as well as the rest single records that were not included into
	 * any linked set. Each evaluated record includes match score to the query record. 
	 * Array is ordered by the match probability.
	 * <p>
	 * If LinkCriteria requires to include query record, then, by the definition
	 * of the linked record set, resulting array will include maximum one set.  Records of this set
	 * and the query record are recommended for merged. The rest records are recommended for a clerical
	 * review.
	 * <p>
	 * If link criteria does not require to include query record then resulting array may include more
	 * then one linked record set. Query record will belong to exacly one set. Records of each set are
	 * recommended to be merged. The rest of the records are recommended for a clerical review.
	 * 
