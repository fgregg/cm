...
public class ServerConfigurationManagerBean implements
	...
	public ServerConfiguration find(long id) {
		ServerConfigurationBean retVal =
			em.find(ServerConfigurationBean.class, id);
		return retVal;
	}
	...
	public MutableServerConfiguration computeGenericConfiguration() {
		MutableServerConfiguration retVal = new ServerConfigurationBean();
	...
	public MutableServerConfiguration clone(ServerConfiguration sc) {
		MutableServerConfiguration retVal = new ServerConfigurationBean(sc);
		return retVal;
	}

	@Override
	public ServerConfiguration save(ServerConfiguration sc)
			throws DuplicateServerConfigurationNameException {
		if (sc == null) {
		...
		ServerConfigurationBean scb = null;
		if (!(sc instanceof ServerConfigurationBean)) {
			scb = new ServerConfigurationBean(sc);
		} else {
			scb = (ServerConfigurationBean) sc;
		}
		assert scb != null;
		...
		if (duplicate != null) {
			assert duplicate instanceof ServerConfigurationBean;
			if (scb.equalsIgnoreIdUuid(duplicate)) {
				...
				throw new DuplicateServerConfigurationNameException(msg);
			}
		}
	...
	public ServerConfiguration setDefaultConfiguration(String host,
		...
		DefaultServerConfigurationBean dsc =
			new DefaultServerConfigurationBean(host, sc.getId());
		em.persist(dsc);
	...
	public ServerConfiguration getDefaultConfiguration(String host,
		...
		ServerConfiguration retVal = null;
		DefaultServerConfigurationBean dscb =
			em.find(DefaultServerConfigurationBean.class, host);
		if (dscb != null) {
		...
		if (retVal == null && computeFallback) {
			...
			assert retVal.getId() != ServerConfigurationBean.NON_PERSISTENT_ID;
