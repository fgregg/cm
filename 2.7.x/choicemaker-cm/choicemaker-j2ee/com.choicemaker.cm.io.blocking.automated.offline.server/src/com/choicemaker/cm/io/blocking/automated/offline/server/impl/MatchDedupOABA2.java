 ...
 * 
 * This version loads one chunk data into memory and different processors handle different trees
 * of the same chunk.  There are N matches files, where N is the number of processors.
 * 
 * For each of the N files, we need to dedup it, then merge all the dedup files together.
 * 
 * @author pcheung
 *
 */
public class MatchDedupOABA2 implements MessageDrivenBean, MessageListener {
	...
	private static final Logger jmsTrace = Logger.getLogger("jmstrace." + MatchDedupOABA2.class.getName());
		...
		try {
			if (inMessage instanceof ObjectMessage) {
				...
				if (o instanceof StartData) {
					...
					//need to merge the deduped temp files when all the proceesors are done
	 ...
	 * @throws JMSException
	 */
	private void handleMerge (Object o) 
		...
		if (BatchJob.STATUS_ABORT_REQUESTED.equals(batchJob.getStatus())) {
			...
			if (d.runTransitivity) sendToTransitivity( new StartData(d));
		}
	}
	
	
	/** This method sends messages to MatchDedupEach to dedup individual match files.
	 * 
	 * @param o
	 * @throws RemoteException
	 * @throws FinderException
	 ...
	 * @throws JMSException
	 */
	private void handleDedupEach (Object o) 
		throws RemoteException, FinderException, BlockingException, NamingException, JMSException {
	...
	private void mergeMatches (int num, long jobID, BatchJob batchJob) 
		...
		IMatchRecord2SinkSourceFactory factory = oabaConfig.getMatchTempFactory();
	...
	/** This sends the message to multiple beans to dedup each of the match temp file created by
	 * the processors.
	 * 
	 * @param jobID
	 * @param percentComplete
	 ...
	 * @throws JMSException
	 */
	protected void sendToMatchDedupEach (StartData d) throws NamingException, JMSException {
