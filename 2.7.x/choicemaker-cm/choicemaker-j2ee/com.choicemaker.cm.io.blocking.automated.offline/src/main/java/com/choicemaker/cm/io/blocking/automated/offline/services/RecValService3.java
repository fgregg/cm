 ...
 *
 */
public class RecValService3<T extends Comparable<? super T>> {
	...
	// This stores if record identifier type is Integer, Long, or string
	...
	public RecValService3(RecordSource stage, RecordSource master,
			...
			IRecordIDTranslator2<T> translator, IStatus status, IControl control) {
				...
				try {
					...
					while (stage.hasNext() && !stop) {
						...
						if (firstStage) {
							Object O = r.getId();
							stageType =
								RecordIdentifierType.checkType(O).typeId;
							firstStage = false;
						}
					} // end while
