 ...
 *
 */
public class MatchToBlockTransformer2<T extends Comparable<? super T>> {

	private static final Logger log = Logger
			.getLogger(MatchToBlockTransformer2.class);

	private IMatchRecord2Source<T> mSource;
	private IMatchRecord2SinkSourceFactory<T> mFactory;
	 ...
	 *            classes.
	 */
	public MatchToBlockTransformer2(IMatchRecord2Source<T> mSource,
			IMatchRecord2SinkSourceFactory<T> mFactory,
			IRecordIDTranslator2<T> translator, IBlockSink blockSink,
	...
	private IMatchRecord2Sink translate() throws BlockingException {
		...
		HashMap<T, Integer> stageIDs;
		HashMap<T, Integer> masterIDs;

		// get the staging id mapping
		List<T> list = translator.getList1();
		assert list != null;
		int size = list.size();
		...
		while (mSource.hasNext()) {
			...
			MatchRecord2<Integer> mr2 =
				new MatchRecord2<Integer>(I1, I2, mr.getRecord2Source(),
	...
	public int[] unionFind(IMatchRecord2Sink sink) throws BlockingException {
		IMatchRecord2Source<T> source = mFactory.getSource(sink);

		SetJoiner setJoiner = new SetJoiner(numRecords);

		source.open();
		while (source.hasNext()) {
			MatchRecord2<T> mr = source.getNext();
	...
	private void writeBlocks(int[] roots) throws BlockingException {
		...
		while (sit.hasNext()) {
			...
			if (size > 2) {
				...
				twos++;
				T C = translator.reverseLookup(set.get(0));

				if (init) {
					idSink.setRecordIDType(RecordIdentifierType.checkType(C).typeId);
					init = false;
				}
