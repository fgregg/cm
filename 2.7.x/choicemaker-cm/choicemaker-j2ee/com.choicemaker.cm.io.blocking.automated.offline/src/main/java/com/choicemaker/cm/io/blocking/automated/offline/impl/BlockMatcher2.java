 ...
 *
 */
public class BlockMatcher2<T extends Comparable<? super T>> implements
		IBlockMatcher2<T> {

	private static final Logger log = Logger.getLogger(BlockMatcher2.class
			.getName());
	private static final Logger logCG = Logger.getLogger(BlockMatcher2.class
			.getName() + ".ComparisonGroup");
	...
	public void matchBlocks(IComparisonArraySource<T> cgSource,
			...
			IMatchRecord2Sink<T> mSink, boolean append, float differ,
			float match, int maxBlockSize) throws BlockingException {
		...
		Map<Object, Record> stageMap = getRecords(stage, stageModel);
		Map<Object, Record> masterMap = getRecords(master, masterModel);
		...
		while (cgSource.hasNext()) {
			...
			ComparisonArray<T> cg = cgSource.getNext();

			if (logCG.isDebugEnabled())
				debugComparisonGroup(cg);
	...
	/*
	 * Print a Comparison Group to the debug log
	 */
	private void debugComparisonGroup(ComparisonArray<T> cg) {
	 ...
	 */
	private void handleBlock(ComparisonArray<T> cg, Map<Object, Record> stage,
			Map<Object, Record> master, ImmutableProbabilityModel model,
	...
	private void handleOversized(ComparisonArray<T> cg,
			Map<Object, Record> stage, Map<Object, Record> master,
		...
		for (int i = 0; i < S.size() - 1; i++) {
			...
			for (int j = i + 1; j < S.size(); j++) {
				T c2 = S.get(j);
				Record m = stage.get(c2);
				log.debug("Round robin s " + c1.toString() + " "
						+ c2.toString());
				MatchRecord2<T> mr =
					compareRecords(clueSet, enabledClues, model, q, m, true,
							low, high);
		...
		for (int i = 0; i < TStage.size(); i++) {
			...
			for (int j = 0; j < S.size(); j++) {
				T c2 = S.get(j);
				Record m = stage.get(c2);
				log.debug("TStage with S " + c1.toString() + " "
						+ c2.toString());
				MatchRecord2<T> mr =
					compareRecords(clueSet, enabledClues, model, q, m, true,
							low, high);
		...
		for (int i = 0; i < TMaster.size(); i++) {
			...
			// q is always stage record
			Record m = master.get(c1);

			for (int j = 0; j < S.size(); j++) {
				T c2 = S.get(j);
				Record q = stage.get(c2);
				log.debug("TMaster with S " + c2.toString() + " "
						+ c1.toString());
				MatchRecord2<T> mr =
					compareRecords(clueSet, enabledClues, model, q, m, false,
							low, high);
		...
		if (TStage.size() <= 4) {
			for (int i = 0; i < TMaster.size(); i++) {
				...
				for (int j = 0; j < TStage.size(); j++) {
					T c2 = TStage.get(j);
					Record q = stage.get(c2);
					log.debug("TMaster random " + c2.toString() + " "
							+ c1.toString());
					MatchRecord2<T> mr =
		...
		for (int i = 0; i < TStage.size(); i++) {
			...
			c2 = TStage.get(n);
			m = stage.get(c2);
			log.debug("TStage random i+1 " + c1.toString() + " "
					+ c2.toString());
			MatchRecord2<T> mr =
				...
				while (count < 3 && j < ids.length) {
					...
					if (ids[j] >= TStage.size()) {
						...
						mr =
							compareRecords(clueSet, enabledClues, model, q, m,
									false, low, high);
						log.debug("TStage random " + c1.toString() + " "
								+ c2.toString());
					} else {
						c2 = TStage.get(ids[j]);
						m = stage.get(c2);
	 ...
	 * @return
	 */
	private static Map<Object, Record> getRecords(RecordSource rs,
			IProbabilityModel model) throws BlockingException {
