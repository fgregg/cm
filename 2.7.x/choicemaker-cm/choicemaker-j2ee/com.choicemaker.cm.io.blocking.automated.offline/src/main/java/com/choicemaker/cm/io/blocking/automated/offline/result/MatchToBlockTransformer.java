...
import com.choicemaker.cm.io.blocking.automated.offline.core.EquivalenceClassBuilder;
 ...
 *
 */
public class MatchToBlockTransformer<T extends Comparable<? super T>> {

	private static final Logger log = Logger
			.getLogger(MatchToBlockTransformer.class);

	private IMatchRecord2Source<T> mSource;
	private IBlockSink blockSink;
	private IRecordIDTranslator2<T> translator;

	private HashMap<T, Long> stageIDs;
	private HashMap<T, Long> masterIDs;
	 ...
	 */
	public MatchToBlockTransformer(IMatchRecord2Source<T> mSource,
	...
	private void populateHashMaps() throws BlockingException {
		...
		while (mSource.hasNext()) {
			MatchRecord2<T> mr = mSource.getNext();
		...
		try {
			...
			while (mSource.hasNext()) {
				...
				if (l1 == null) {
					...
					log.error("l1 == l2 " + mr.getRecordID1() + " "
							+ mr.getRecordID2());
				}
	...
	private void writeBlocks(EquivalenceClassBuilder builder)
		...
		SortedSet<EquivalenceClass> set = builder.getEquivalenceClasses();
		Iterator<EquivalenceClass> it = set.iterator();
		...
		while (it.hasNext()) {
			...
			int size = ec.size();
			if (size > 2) {
				Iterator<Long> it2 = ec.getMemberIds().iterator();
