...
/**
 * This object uses a group of IBlockSinks to store the blocks.  It uses at least one sink for
 * blocks of a given size.
 * 
 * This objects makes it faster to run blocks dedup, because it doesn't have to split the blocks.
 * 
 * @author pcheung
 *
 */
public class BlockGroup implements IBlockSink {
	...
	// if there is more than 1 bucket file for an id, we need to rotate the bucket files.
...
//	private int maxSize; //maximum size of a block set.
	 ...
	 * 
	 * @param bFactory - IBlockSinkSourceFactory that provides sinks for different block size.
...
//		this.maxSize = maxSize;
	...
	/** This returns an array of IBlockSource built from the IBlockSinks.  The ordering is the file
	 * containing the smallest blocks to the file with the largest blocks.
	 * 
	 * @return IBlockSource [] - an array of IBlockSource. All the blocks in a source all have the same
	 * size.
	 * @throws IOException
	 */
	...
	/** This returns an array of IBlockSource built from the IBlockSinks.  The ordering is the file
	 * containing the largest blocks to the file with the smallest blocks.
	 * 
	 * @return IBlockSource [] - an array of IBlockSource. All the blocks in a source all have the same
	 * size.
	 * @throws IOException
	...
	public IBlockSource [] getSourcesReverse () throws BlockingException {
		...
		int j = sinks.size() - 1;
		for (int i=0; i< sinks.size(); i++) {
			sources[i] = bFactory.getSource((IBlockSink) sinks.get(j) );
			j --;
		}
	...
	/* (non-Javadoc)
	 * @see com.choicemaker.cm.io.blocking.automated.offline.core.IBlockSink#writeBlock(com.choicemaker.cm.io.blocking.automated.offline.core.BlockSet)
	 */
