...
//	private int maxBlockSize;
	 ...
	 */
	public BlockDedupService (BlockGroup bGroup, IBlockSink bSink, int maxBlockSize, OabaEventLog status) {
		this.bGroup = bGroup;
		this.bSink = bSink;
//		this.maxBlockSize = maxBlockSize;
	...
	public void runService () throws BlockingException {
		...
		if (status.getCurrentOabaEventId() >= OabaProcessing.EVT_DONE_DEDUP_BLOCKS ) {
			...
			int startPoint = Integer.parseInt( status.getCurrentOabaEventInfo() ) + 1;
...
//				System.out.println (i + " Processing: " + source.getInfo() + " count: " + numBlocksIn);
				...
				if (MemoryEstimator.isFull(FULL) || (i == parts.length  - 1)) {
					...
					status.setCurrentOabaEvent(OabaEvent.DEDUP_BLOCKS, Integer.toString(i) );
