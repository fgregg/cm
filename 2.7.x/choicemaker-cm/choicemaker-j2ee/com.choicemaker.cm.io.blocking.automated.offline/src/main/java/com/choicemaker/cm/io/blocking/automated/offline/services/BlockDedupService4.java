...
public class BlockDedupService4 {
	...
	private static final Logger log = Logger
			.getLogger(BlockDedupService4.class);
	...
	public BlockDedupService4(BlockGroup bGroup,
			...
			int maxBlockSize, IStatus status, IControl control, int interval) {
	...
	private void startDedup2(int startPoint) throws BlockingException {
		...
		List<IBlockSink> bigBlocks = new ArrayList<>();
		...
		while (pair[0] > 0 && !stop) {
			...
			List<IBlockSink> stack = new ArrayList<>();

			// read the files in size ascending order.
			int blockSetId = 0;

			// this is needed for writing out distinct blocks
			List<IBlockSource> sources = new ArrayList<>();
			...
			/*
			 * //debug if (log.isDebugEnabled()) { ArrayList children =
	...
	private int runDedup(IBlockSource[] parts, int from, int to,
			List<IBlockSource> sources, int blockSetId, IntArrayList subsumedBlockSets,
	...
	private void compareToBig(SuffixTreeNode root,
			IntArrayList subsumedBlockSets, List<IBlockSink> bigBlocks)
			throws BlockingException {

		int c = 0;

		for (int i = 0; i < bigBlocks.size() && !stop; i++) {
			IBlockSink sink = bigBlocks.get(i);
	...
	private int writeUnsubsumedTemp(IntArrayList subsumedBlockSets,
			IBlockSink sink, List<IBlockSource> sources) throws BlockingException {
	...
	private void writeTree(SuffixTreeNode root, ISuffixTreeSink sink2)
		...
		List<SuffixTreeNode> branches = root.getAllChildren();
