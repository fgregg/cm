 ...
 *
 */
public class GenericDedupService<T extends Comparable<? super T>> {

	private static final Logger log = Logger
			.getLogger(GenericDedupService.class);

	private static final int INTERVAL = 1000;

	private IComparableSource<T> cSource;
	private IComparableSink<T> cSink;
	private IComparableSinkSourceFactory<T> cFactory;
	private int max;

	// list of temporary sinks
	private List<IComparableSink<T>> tempSinks = new ArrayList<>();
	 ...
	 * Comparables, and a factory to generate temp sinks. max is the maximum
	 * size of the TreeSet.
	 * 
	 * @param mSource
	 * @param mSink
	 ...
	 * @param status
	 */
	public GenericDedupService(IComparableSource<T> cSource,
			IComparableSink<T> cSink, IComparableSinkSourceFactory<T> cFactory,
	...
	public void runDedup() throws BlockingException {
		...
		if (numBefore == 0) {
			...
			for (int i = 0; i < max; i++) {
				IComparableSink<T> tempSink = tempSinks.get(i);
	...
	private void writeToFiles() throws BlockingException {
		...
		IComparableSink<T> tempSink = cFactory.getNextSink();
		tempSinks.add(tempSink);
		tempSink.open();

		cSource.open();

		log.debug("Reading from " + cSource.getInfo() + " writing to "
				+ tempSink.getInfo());
	 ...
	 */
	public static <T extends Comparable<? super T>> int mergeFiles(
			List<IComparableSink<T>> tempSinks, IComparableSink<T> cSink,
			IComparableSinkSourceFactory<T> cFactory, boolean delete)
			throws BlockingException {
		...
		if (numFiles == 1) {
			IComparableSink<T> snk = tempSinks.get(0);
		...
		IComparableSource<T>[] sources = new IComparableSource[numFiles];
		@SuppressWarnings("unchecked")
		T[] comps = (T[]) new Object[numFiles];

		for (int i = 0; i < numFiles; i++) {
			sources[i] = cFactory.getSource(tempSinks.get(i));
			sources[i].open();
	...
	private static <T extends Comparable<? super T>> int count(
			IComparableSink<T> snk, IComparableSinkSourceFactory<T> cFactory)
			throws BlockingException {

		int i = 0;
		IComparableSource<T> source = cFactory.getSource(snk);
		source.open();
	 ...
	 * @return
	 */
	public static <T extends Comparable<? super T>> int nonOverlap(
			IComparableSource<T> source, IComparableSource<T> dups,
			IComparableSink<T> cSink) throws BlockingException {
	 ...
	 */
	private static <T extends Comparable<? super T>> T getNextComp(
			IComparableSource<T> dups) throws BlockingException {
		T nextComp = null;
		if (dups.hasNext()) {
	 ...
	 */
	private static <T extends Comparable<? super T>> T getNextCompBiggerThan(
			IComparableSource<T> dups, T c) throws BlockingException {
		T nextComp = null;
		boolean stop = false;
	 ...
	 * @return
	 */
	private static <T extends Comparable<? super T>> int findMin(T[] records,
			int size) {
		T min = null;
