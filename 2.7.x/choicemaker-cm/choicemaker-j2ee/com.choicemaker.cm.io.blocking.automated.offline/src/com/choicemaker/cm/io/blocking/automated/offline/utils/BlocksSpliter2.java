...
public class BlocksSpliter2 {
	...
	public BlocksSpliter2 (IBlockSinkSourceFactory bFactory, int minSize, int maxSize, int numFiles) {
		...
		this.interval = (int) Math.round( Math.ceil((maxSize - minSize) * 1.0/numFiles));
...
//		System.out.println (minSize + " " + maxSize + " " + numFiles + " " + interval);
	}
	
	
	/** This method sets the number of bucket files to create for the given interval.
	 * By default there is 1 bucket file per interval, but you can increase that if there are
	...
	public void setSize (int interval, int num) {
		if (num >= 0 && num < numFiles) numBuckets [interval - 1] = num;
	}
